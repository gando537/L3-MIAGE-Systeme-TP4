...
int main(int argc, char *argv[]){
  ...
  char msg_in[MAX_DGRAM_SIZE];     /* Message recu de "0" a "99" */
  ...
  //  struct sockaddr_in adr2, *padr2 = &adr2; /* Inutilise pour l'instant */
    ...
    if (sendto(sd_local,msg_out, MAX_DGRAM_SIZE, 0, (struct sockaddr *)padr_dist, ls) <1)
      printf("inacheve : %s !\n",msg_out);
    /* b) Reception */
    if (recvfrom(sd_local,msg_in, MAX_DGRAM_SIZE, 0, (struct sockaddr *)NULL, NULL) == -1)
      printf("inachevee : %s !\n",msg_in);
    else  {
      printf("resultat = %s ! \n",msg_in);
    }
